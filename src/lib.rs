...
use crate::hooks::{
    race_info_hook,
    team_stadium_result_hook,
    ORIG_GET_RACE_TRACK_ID,
    ORIG_TEAM_STADIUM_RESULT,
    API_HOOK_FNS,
    API_HOOK_ORIGS,
    MAX_API_HOOKS,
};
...
unsafe fn install_hooks() {
    ...
    let results = find_methods_in_assembly_by_param(target_image as *mut RawIl2CppImage, "CommonResponse");
    if results.is_empty() {
